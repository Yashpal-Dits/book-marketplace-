...
export const sellerBookRequestSchema = Yup.object({
  ...
  publisher: Yup.string()
    .trim()
    .min(2, 'Publisher is too short')
    .max(80, 'Publisher is too long')
  ...
  coverImageFile: Yup.mixed<File>()
    .nullable()
    .test('file-size', 'Image must be 5MB or smaller', (file) => {
      if (!file) return true
      return file.size <= MAX_IMAGE_SIZE
    })
    .test('file-type', 'Only JPG, PNG, WEBP or GIF images are allowed', (file) => {
      if (!file) return true
