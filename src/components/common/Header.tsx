...
const pendingSellerNavLinks = [
  { to: '/', label: 'Home' },
  { to: '/seller/pending-approval', label: 'Approval Pending' },
]
...
export const Header = () => {
  ...
  const navLinks = isImpersonatingCustomer
    ...
    : user?.role === Role.ADMIN
      ? adminNavLinks
      : isPendingSeller
        ? pendingSellerNavLinks
        : user?.role === Role.SELLER
          ? sellerNavLinks
          : customerNavLinks

  const shouldShowCart = shouldUseCart

  const linkClass = ({ isActive }: { isActive: boolean }) =>
    cn(
      'text-sm font-medium transition hover:text-emerald-400',
      isActive ? 'text-emerald-400 underline underline-offset-4' : 'text-white/85',
    )
        ...
        <div className="flex h-16 w-full items-center justify-between gap-4 px-6 sm:px-8 lg:px-12">
          <div className="flex flex-1 items-center gap-4">
            <button
              ...
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition hover:bg-white/10 lg:hidden"
            >
              <FiMenu size={20} />
            </button>
            ...
            <Link to="/" className="flex items-center gap-2.5 whitespace-nowrap">
              ...
              <span className="font-display text-xl font-extrabold tracking-tight text-emerald-300 sm:text-2xl">
                Bseller
              </span>
            </Link>
          </div>
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/5 transition hover:bg-red-500/20"
                  title="Logout"
                >
                  <FiLogOut size={18} />
                </button>
              ...
              <Link
                to="/login"
                className="rounded-full bg-primary px-4 py-2 text-xs font-bold uppercase tracking-wider transition hover:bg-primary-hover"
              >
                Login
              </Link>
            )}
